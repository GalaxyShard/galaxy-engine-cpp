...
bool cube0_collides_with(CubeCollider *cube0, CubeCollider *cube1)
{
    ...
    Vector3 leftDownClose = inverseRot*(cube1->pos + radius*Vector3(-1,-1,1));
    Vector3 rightDownClose = inverseRot*(cube1->pos + radius*Vector3(1,-1,1));
    Vector3 leftUpClose = inverseRot*(cube1->pos + radius*Vector3(-1,1,1));
    Vector3 rightUpClose = inverseRot*(cube1->pos + radius*Vector3(1,1,1));

    Vector3 leftDownFar = inverseRot*(cube1->pos + radius*Vector3(-1,-1,-1));
    Vector3 rightDownFar = inverseRot*(cube1->pos + radius*Vector3(1,-1,-1));
    Vector3 leftUpFar = inverseRot*(cube1->pos + radius*Vector3(-1,1,-1));
    Vector3 rightUpFar = inverseRot*(cube1->pos + radius*Vector3(1,1,-1));
